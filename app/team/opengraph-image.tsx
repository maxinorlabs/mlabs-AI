    ...
    (
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <span style={{ fontSize: '14px', fontWeight: 700, color: '#F36F21', letterSpacing: '0.2em', textTransform: 'uppercase', fontFamily: 'sans-serif' }}>Team</span>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '20px' }}>
            <div style={{ width: '5px', height: '72px', background: '#F36F21', borderRadius: '4px', flexShrink: 0, marginTop: '6px' }} />
            <span style={{ fontSize: '62px', fontWeight: 700, color: '#F7F7F5', lineHeight: 1.1, fontFamily: 'sans-serif', letterSpacing: '-0.02em' }}>
              Meet the Operator Team
            </span>
          </div>
          <p style={{ fontSize: '24px', color: 'rgba(247,247,245,0.5)', fontFamily: 'sans-serif', fontWeight: 400, margin: 0, paddingLeft: '25px' }}>
            Experienced operators driving founder success through hands-on execution
          </p>
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div style={{ display: 'flex', gap: '12px' }}>
            {['Leadership', 'Entrepreneurs in Residence', 'Program Team'].map((tag) => (
              <div key={tag} style={{ background: 'rgba(243,111,33,0.15)', border: '1px solid rgba(243,111,33,0.3)', borderRadius: '100px', padding: '8px 18px', fontSize: '14px', color: '#F36F21', fontFamily: 'sans-serif', fontWeight: 600 }}>
                {tag}
              </div>
            ))}
          </div>
